...
class RedisSaver(BaseCheckpointSaver):
    ...
    def from_conn_info(cls, *, host: str, port: int, db: int) -> Iterator["RedisSaver"]:
        conn = None
        try:
            conn = Redis(host=host, port=port, db=db,ssl=True,password=os.getenv("pass"))
    ...
    def get_tuple(self, config: RunnableConfig) -> Optional[CheckpointTuple]:
        ...
        return _parse_redis_checkpoint_data(
            self.serde, checkpoint_key, checkpoint_data, pending_writes=None
        )
    ...
    def delete(self, config: RunnableConfig) -> None:
        ...
        Args:
            config (RunnableConfig): The config of the checkpoint to delete.
        """
        thread_id = config["configurable"]["thread_id"]
        checkpoint_ns = config["configurable"].get("checkpoint_ns", "")
        checkpoint_id = get_checkpoint_id(config)
        ...
        deleted_count = self.conn.delete(checkpoint_key)


        # Also delete any associated writes
        # writes_key_pattern = _make_redis_checkpoint_writes_key(
        #     thread_id, checkpoint_ns, checkpoint_id, "*", None
        # )
        keys = _filter_keys(self.conn.keys(pattern), before=None, limit=None)
        for key in keys:
            deleted_count = self.conn.delete(key)
